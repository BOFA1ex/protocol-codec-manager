...
public class AESUtils {
     ...
     * @return aes加密后的数据
     */
    public static byte[] encrypt_cbc(byte[] data, byte[] key, boolean isPadding) {
        try {
     ...
     * @return aes解密后的数据
     */
    public static byte[] decrypt_cbc(byte[] data, byte[] key, boolean isPadding) {
        try {
            SecretKeySpec keySpec = new SecretKeySpec(key, "AES");
            byte[] iv = ByteUtils.copyOfRange(data, 0, 16);
            byte[] realData = ByteUtils.copyOfRange(data, 16, data.length);
            IvParameterSpec ivps = new IvParameterSpec(iv);
            Cipher cipher;
            if (isPadding) {
                cipher = Cipher.getInstance("AES/CBC/PKCS7Padding", BouncyCastleProvider.PROVIDER_NAME);
     ...
     * @return aes加密后的数据
     */
    public static byte[] encrypt_ecb(byte[] data, byte[] key, boolean isPadding) {
        try {
     ...
     * @return aes解密后的数据
     */
    public static byte[] decrypt_ecb(byte[] data, byte[] key, boolean isPadding) {
        try {
            SecretKeySpec spec = new SecretKeySpec(key, "AES");
            Cipher cipher;
            if (isPadding) {
